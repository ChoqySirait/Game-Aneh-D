...
function initMemory(canvas) {
  ...
  startMemoryLevel(canvas);
}

function startMemoryLevel(canvas) {
  let s = memoryState;
  s.phase = 'SHOW';
...
function renderMemory(ctx, canvas) {
  let s = memoryState;
  ...
  for (let r = 0; r < s.gridSize; r++) {
    for (let c = 0; c < s.gridSize; c++) {
      ...
      if (s.phase === 'SHOW' && isActive) {
        ctx.shadowBlur = 30; ctx.shadowColor = "#00f2fe";
        ctx.fillStyle = "#00f2fe";
      } else if (s.phase === 'GUESS' && isSelected) {
        if (isActive) {
          ctx.shadowBlur = 30; ctx.shadowColor = "#00e676";
...
function handleMemoryClick(x, y, canvas) {
  let s = memoryState;
    ...
    if (!s.selectedTiles.includes(index)) {
      ...
      if (!s.activeTiles.includes(index)) {
        ...
        if (correctCount === s.activeTiles.length) {
          s.score += 50;
          s.level++;
          setTimeout(() => startMemoryLevel(canvas), 800);
        }
      }
    }
  }
